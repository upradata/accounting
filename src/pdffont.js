...
let nodeUtil = require("util"),
    _ = require("lodash"),
    PDFUnit = require('pdf2json/lib/pdfunit.js');
...
let PDFFont = (function PFPFontClosure() {
    ...
    let cls = function (fontObj) {
        ...
        this.spaceWidth = this.spaceWidth;

        this.fontObject = fontObj;
        this.emSize = this.charSize('M');
    };

    cls.prototype.charSize = function (char) {
        const index = this.fontObject.toUnicode.indexOf(char);

        if (index === -1) {
            console.warn(`${char} does not have a width set in the font ${this.fontObject.name}`);
            return 0;
        }

        const { widths } = this.fontObject;
        let i = index;
        while (i > 0) {
            if (widths[i] > 0)
                break;
    ...
    cls.prototype.gridUnitToPixel = function (size, fontSizePt) {
        return (size / this.emSize) * (fontSizePt / 72) * 96; // 96 = dpi = pixels/in
    };


    cls.prototype.charWidthInPixel = function (char, fontSizePt) {
        return this.gridUnitToPixel(this.charSize(char), fontSizePt);
    };
    ...
    cls.prototype.processText = function (p, str, maxWidth, color, fontSize, targetData, matrix2D) {
        ...
        let oneText = {
            ...
            font: this.fontObj,
            emSize: this.emSize,
            emPixels: this.fontSize * 96 / 72, // fontSize= 24 points for instance
            spaceWidth: this.spaceWidth,
            spaceWidthPixels: this.charWidthInPixel(' ', fontSize),
